...
spa.chat = ( function () {
    ...
    var
        configMap = {
            ...
            slider_closed_title : 'Clict to open',
            chat_model          : null,
            people_model        : null,
    ...
    setJqueryMap = function () {
        ...
        jqueryMap = {
            ...
            $toggle : $slider.find( 'spa_chat-head-toggle' ),
    ...
    setPxSizes = function () {
        var px_per_em, window_height_em, opened_height_emn;
    ...
    //  このコールバックは単一引数としてスライダーDIVを表すｊQueryコレクションを受け取る」
    // 動作：スライダーを要求された位置に移動する
    ...
    setSliderPosition = function ( position_type, callback ) {
        ...
        // スライダーがすでにようきゅされた位置にある場合はtrueを返す
    ...
    //    やりとりをするメソッドをていきょうするチャットモデルオブジェクト
    //  * people_model  モデルが保持する人々のリストを管理するメソッドを
    //    提供するピープルモデルオブジェクト
    //  * slider_*構成    すべてのオプションのスカラー。完全なリストはmapConfig_mapを参照
    ...
    // 目的：
    //  ウインドウリサイズイベントに対し、必要に応じてこのモジュールが提供する表示を調整する
    // 動作：
    //  ウインドウの高さや幅が所定の閾値を下回ったら、
    //  縮小したウインドウサイズに合わせてチャットスライダーのサイズを変更する
    // 戻り値：プール値
    //  * false リサイズを考慮していない
    //  * true  リサイズを考慮した
    // 例外発行：なし
